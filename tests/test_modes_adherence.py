...
EXPECTED_BUNDLE_MD = textwrap.dedent(
    """\
    ---
    bundle:
      name: bundlewizard
...
def test_bundle_md_matches_two_track_public_surface():
    """bundle.md must publish the contained two-track UX surface exactly."""
    assert BUNDLE_MD.exists(), "bundle.md does not exist"
    assert BUNDLE_MD.read_text(encoding="utf-8") == EXPECTED_BUNDLE_MD
